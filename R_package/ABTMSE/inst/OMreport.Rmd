...
matplot(OMI@Hyears[1]:OMI@years[2],t(allSSB)/1E6,ylim=c(0,max(dynB0)/1E6),type='l',yaxs='i',col="white",xlab="Year",ylab="SSB (1000 t)")
...
for(pp in out$np:1){
  ...
  plot(yrs,muSSB[pp,]/1000,lwd=2,ylab="",type="l",ylim=ylim,yaxs='i')
  ...
  plot(yrs,Rec[pp,],lwd=2,ylab="SSB(t)",type="l",ylim=ylim,yaxs='i')
  cond<-tdat$Assessment=="SS"
  lines(tdat$Year[cond],as.numeric(as.character(tdat$Val[cond])),col="#ff000090",lwd=2)
  lines(tdat$Year[!cond],as.numeric(as.character(tdat$Val[!cond])),col="#00ff0090",lwd=2)
...
plot(yrs,muSSBc/1000,lwd=2,ylab="",type="l",ylim=ylim,yaxs='i')
cond<-tdat$Assessment=="SS"
lines(tdat$Year[cond],as.numeric(as.character(tdat$Val[cond])),col="#ff000090",lwd=2)
lines(tdat$Year[!cond],as.numeric(as.character(tdat$Val[!cond])),col="#00ff0090",lwd=2)
...
plot(yrs,Recc,lwd=2,ylab="",type="l",ylim=ylim,yaxs='i')
cond<-tdat$Assessment=="SS"
lines(tdat$Year[cond],as.numeric(as.character(tdat$Val[cond])),col="#ff000090",lwd=2)
lines(tdat$Year[!cond],as.numeric(as.character(tdat$Val[!cond])),col="#00ff0090",lwd=2)
if(pp==out$np)mtext("Recruitment",2,line=2.8,font=2)
...
Figure 5. Regional comparisons (45deg W) with 2017 stock assessment. Note that annual estimates from the operating model are calculated from average of the seasonal predictions.  
...
Figure 6. Model predicted spawning stock biomass and recruitment. h is the assumed value of steepness, lnR0 is the log unfished recruitment. In the case of the hockey stick SR model, Inflec. is the biomass position of the inflection (join) point relative to unfished.Hockey-stick stock-recruitment curves define the inflection point as the mean biomass in the range of 1990-1995, and these depicted by red points.
...
#if(nplot==2)par(mfcol=c(1,2),mai=c(0.4,0.4,0.01,0.01),omi=c(0.7,0.7,0.7,0.01))
#if(nplot>2)par(mfcol=c(2,2),mai=c(0.4,0.4,0.01,0.01),omi=c(0.7,0.7,0.7,0.01))
...
for(i in 1:nplot){
  ...
  SSB<-resid[[i]]$SSB[ind]
  #plot(SSB,modrec)
  #res<-log(((resid[[i]]$rec-resid[[i]]$devs)/resid[[i]]$rec))

  res0<-(estrec-modrec)/modrec
  yrs<-resid[[i]]$yrs[ind]
...
BMSY<-round(res[,6]*dynB0[,nHy+ny]/1000,0)
...
MSY<-c( (1-F_EinW[ny])*res$MSY[1]+F_WinE[ny]*res$MSY[2], 
            F_EinW[ny]*res$MSY[1]+(1-F_WinE[ny])*res$MSY[2] )
BMSY<-c( (1-F_EinW[ny])*res$BMSY[1]+F_WinE[ny]*res$BMSY[2], 
            F_EinW[ny]*res$BMSY[1]+(1-F_WinE[ny])*res$BMSY[2] )
BMSY_B0<-c( (EinE[ny]*res$BMSY_B0[1]+WinE[ny]*res$BMSY_B0[2])/sum(EinE[ny],WinE[ny]),
             (EinW[ny]*res$BMSY_B0[1]+WinW[ny]*res$BMSY_B0[2])/sum(EinW[ny],WinW[ny]) )
B_BMSY<-c( (EinE[ny]*res$B_BMSY[1]+WinE[ny]*res$B_BMSY[2])/sum(EinE[ny],WinE[ny]),
             (EinW[ny]*res$B_BMSY[1]+WinW[ny]*res$B_BMSY[2])/sum(EinW[ny],WinW[ny]) )
Dep<-c( (EinE[ny]*res$Dep[1]+WinE[ny]*res$Dep[2])/sum(EinE[ny],WinE[ny]),
             (EinW[ny]*res$Dep[1]+WinW[ny]*res$Dep[2])/sum(EinW[ny],WinW[ny]) )
...
Fprof<-meanFs(FML=out$FL[out$ny,,,,], iALK=out$iALK[,out$ny,,], N=out$N[,out$ny,,,],
         wt_age=t(out$wt_age[out$ny,,]))
matplot(t(Fprof),type='l',xlab="Age",ylab="F",lty=c(1,1),col='white',yaxs='i')
...
Figure 8. The current profile of F at age summed over all fleets, average weighed (by total contribution to annual F) over seasons and areas. 
...
Figure 9. The size selectivity of the various fishing fleets included in the operating model. 
...
```{r catchaggfit, fig.width=8.5, fig.height=11,echo=FALSE}
  ...
  barplot(tempobs,ylim=c(0,max(tempobs,temppred)),names.arg=c(OMI@Fleets$name,"OTH"),col="dark grey",las=2,border=NA)#sum,na.rm=T),pch=19)
...
Figure 10. Fit to a observed total catches (tons) aggregated over various axes (lines = predicted, points = observed)
...
Figure 11.Length composition residuals. The left-hand panels show residuals over time. Orange circles are positive residuals (observed proportion is higher than expected), blue circles are negative residuals. The area of the circle is proportional to the absolute residuals error (i.e. if the observed proportion of a length class is 0.5 and the model predicted fraction is 0.4, this is represented by a circle ten time larger than if the observed proportion was 0.05 and the predicted fraction is 0.04). This keeps the scaling of the left-hand plots the same as the right-hand plots that show the aggregate residual error over all years and (right mosts panels) the period since 2007. Given the estimated selectivity of each fleet, the grey lines show the predicted composition of catches for a hypothetical unfished Eastern stock (solid grey line) and a hypothetical unfished western stock (dashed grey line).
...
```{r fit to agg length observations2, fig.width=7.5, fig.height=9,echo=FALSE}
  ...
  for(ff in 8:nf){
    ...
    #legend('topright',legend=paste("n=",round(sum(CLobsA[,,,ff,],na.rm=T)/1000,1),"k",sep=""),bty='n',cex=0.9)
      if(ff==8)legend('topright',legend="All years",bty='n') 
    ylim=c(0,max(CLoa10[ff,],CLpa10[ff,],UFfreq[,ff,]))
    barplotT(out$ml,CLoa10[ff,],ylim=ylim)
    #plot(CLoa[ff,],ylim=ylim,col='orange',pch=19,cex=2)
...
Figure 11 cont. Length composition residuals. 
...
```{r mixing, fig.width=8, fig.height=5,echo=FALSE}

par(mfrow=c(1,2),mai=c(0.8,0.8,0.1,0.15),omi=c(0.1,0.1,0.4,0.01))
...
Figure 16. Predicted stock mixing (averaged over seasons, summed over areas). On the left is the degree to which each stock has been distributed in the opposite ocean area over time. One the right is the fraction of Eastern Stock biomass found in the East/West areas over time (stock composition over time). 
...
Figure 17. Predicted stock mixing by year, season and area. The red line is the fraction (%) of stock biomass that is Eastern.  
