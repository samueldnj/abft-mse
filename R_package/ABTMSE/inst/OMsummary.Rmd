...
names(DM)<-c("Code.","Future Recruitment","Current Abundance","Maturity / M")
DM%>%
...
for(i in 1:nOMs){
  ...
  BMSY<-round(res[,6]*dynB0[i,,nHy+ny]/1000,0)
...
```{r SSBa_overlap, results='asis',echo=FALSE,fig.width=9,fig.height=6}
  ...
  cols=rep(c('black','red','green','blue'),each=2)
  ltys=rep(1:2,20)
  ...
  for(aa in 2:1){
    ...
    matplot(yrs,t(SSBstore[,aa,]/1000),col=cols,lty=ltys,type='l',ylab="",ylim=c(0,quantile(SSBstore[,aa,]/1000,0.99)))
    mtext(paste(anams[aa],"Stock"),3,line=0.5)
   
    matplot(yrs,t(SSBastore[,aa,]/1000000),col=cols,lty=ltys,type='l',ylab="",ylim=c(0,quantile(SSBastore[,aa,]/1000000,0.99)))
    ...
    if(aa==1) legend('bottom',legend=c("Rec 1","Rec 2","Mat/M I","Mat/M II","Mat/M III","Mat/M IV"),lty=c(1,2,1,1,1,1),col=c("black","black",cols[c(1,3,5,7)]),bty='n')
...
LHs<-LHsU<-data.frame(array(NA,c(nOMs,16)))
names(LHs)<-names(LHsU)<- c("OM","Code","Catch", "CPUE", "Survey", "L Comp", "SOO", "PSAT", "Rec Dev", "Mov Pr", "Sel Pr", "SRA pen", "SSB Pr", "Dep Pr",  "MI Pr","TOT")

wts<-OMIs[[i]]@LHw[c(1:6,8:14)] # 13 
LHs[,1]<-LHsU[,1]<-OMnos
LHs[,2]<-LHsU[,2]<-OMnames

for(i in 1:nOMs){
  
  LHs[i,3:16]<-outs[[i]]$obj[c(1:13,15)] # ignore R0 penalty for now
  LHsU[i,3:15]<-outs[[i]]$obj[1:13]/wts
  
  
  LHs[i,is.na(LHs[i,])|LHs[i,]=='Inf']<-0
  LHs[i,3:16]<-round(LHs[i,3:16],1)
  LHsU[i,is.na(LHsU[i,])|LHsU[i,]=='Inf']<-0
  LHsU[i,16]<-sum(LHsU[i,3:15])
  LHsU[i,3:16]<-round(LHsU[i,3:16],0)
}  
...
Catch = catch data by fleet, quarter and area. CPUE = the fishery dependent CPUE indices, Survey = fishery-independent indices, L Comp = length composition data, SOO = stock of origin data, PSAT = Electronic tagging data, Rec Dev = prior on recruitment deviations, Mov Pr = prior on movement parameters, Sel Pr = prior on size selectivity parameters, SRA pen = penalty incurred when catches exceed F=1 catches in the stock reduction analysis phase (1864-1964), SSB Pr = a prior on spawning stock biomass by area for reference operating models B (match VPA assessments), Dep Pr = a prior on stock depletion since 1980 to generate more depleted stocks for refernece operating models C (historically low stock levels), MI Pr = a prior on similarlity to the 'Master Index' that predicts F by year, area, season and fleet, TOT = total global objective function. 
