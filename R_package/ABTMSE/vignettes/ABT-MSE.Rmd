---
title: "ABT-MSE: Atlantic Bluefin Tuna Management Strategy Evaluation (v4.2.9)"
...
The Atlantic Bluefin Tuna Management Strategy Evaluation is funded by the ICCAT GBYP (coordinated by Francisco Alemany <francisco.alemany@iccat.int>). The technical aspects of this modelling work are overseen by the GBYP MSE technical team including David Die, Doug Butterworth, Shuya Nakatsuka, Harritz Arrizabalaga, Ai Kimoto, Toshi Kitakado, Miguel Santos, John Walter, Carmen Fernandez. Previous collaborators include Paul de Bruyn, Yukio Takeuchi, Laurie Kell, Clay Porch, Joe Powers and Campbell Davies. 
...
The package is subject to ongoing testing and development. If you find a bug or a problem please send a report to <t.carruthers@oceans.ubc.ca> so that it can be fixed!  


## Version 4.2.9

* 12 Robustness operating models now included in the Package that are 5 deviations from the first two reference operating models 1AI and 2AI: ROM_1_1, ROM_1_2, ROM_2_1, ROM_2_2, ROM_3_1, ROM_3_2, ROM_4_1, ROM_4_2, ROM_5_1, ROM_5_2, ROM_6_1 and ROM_6_2. 

## Version 4.1.2 

* Fix for erroneous future index calculation for indices that have data after the last year of conditioning (2017)

## Version 4.1.1 

* Overhauled Operating models
* Two-phase recruitment estimation in level 1 and 3 recruitment
* Operating models fitted to data up to 2016

## Version 3.4.1 

* Trade-off plot added to shiny App

## Version 3.4.0 

* Includes new Shiny App. Shiny(ABTMSE)

## Version 3.3.3 

* UCT version without kableExtra package.

## Version 3.3.2 

* MP recommendations were erroneously starting in 2018 (model year 54), this has been corrected to 2019 (model year 55)

* OMs for recruitment level 3 were not included in some packages. These are all included now.  
...
* Bug in stock-recruitment code block was chronically lowering productivity
* getperf(MSE_example,quantile=T) now works as expected

## Version 3.2.8

* UCT version without kableExtra dependency

## Version 3.2.7

* Debug of plot(MSE) now provides correct SSB/SSBMSY, e.g.  plot(MSE_Example, MSY=T)
...
* Updated plot(MSE) defaults to plotting SSB/SSBMSY, e.g.  plot(MSE_Example, MSY=T)
* New performance metric Brel30 - Spawning biomass relative to dynamic SSBMSY after 30 years 
* New slots have been added to the MSE object that record MSY related quantities (matrices nsim rows by stock columns):  MSY, BMSY, VBMSY,SSBMSY, UMSY, FMSYa, SSBMSY_SSB0,
...
* Updated MSE plots to start in 2019

## Version 3.2.2

* Removed all Factor 2 level C (low depletion) scenarios from the operating model objects in the package. 
* Some text typos fixed

## Version 3.2.1

* UCT version - removed kableExtra temporarily for UCT cluster computation on < OpenSSL v1.0

## Version 3.2

* Updated performance metrics to include 'dynamic B0' a theoretical running biomass with zero fishing (smoothly tracks changes in recruitment strength)

* Added correct lags in data (y-3 for catches, y-2 for indices, y-2 catch data are assumed to be the TAC that was set)

* New indices to future generated data for MP use (Canadian Acoustic Survey and US RR 66-114)

* Various plotting fixes for plot(MSE)

* New generic MSE reporting function (create a standardized html document): MSE_report(MSE_example,dir="C:/temp")


## Version 3.1

* All operating models revised and refitted in light of trial specifications - both factors 1 (recruitment) and 2 (abundance) changed.


## Version 3.0

* Shift to multi-periodic R0 recruitment model. E.g. Beverton-Holt before 1975, hockey stick with SSB hingpoint in the mid 90's after 1975

* New log normal likelihood function for composition data where precision is the inverse of the composition fraction (of Maunder 2013)

* New logit likelihood function for stock of origin data

* Use of alternative stock of origin data (derived by mixture modelling of microchemistry assignment data - Carruthers and Butterworth 2018)


## Version 2.8

* getperf()  now allows users to report quantiles of all the metrics: getperf(myMSE,quantile=T)

* The D10, D20 and D30 metrics have been fixed to reflect spawning stock depletion after 10, 20 and 30 years respectively. 

* An error reported by JJ Maguire has been fixed relating to the variable 'Cunder' which controls over take of catches in areas where there is insufficient yield

* If users wish to use East and West data simultaneously to set TACs in just one area then they can do so accepting dset to their MP (now with two positions: 1 is the East and 2 is the West). They must include an argument AS which is the area that they are providing advice for. Also they must make their class of MP 'MSMP' for Multi-Stock Management Procedure. See the MSMP named CSMP for an example: avail('MSMP') 

## Version 2.7

All operating models now have a deterministic (no process error) version that includes just two identical simulations at the MLE estimate of model parameters. Previous operating models are named OM_1, OM_2, OM_R1, the deterministic version have a 'd' on the end: OM_1d, OM_2d, OM_R1d... To generate almost identical future projections use the Perfect_Obs observation model.  

## Version 2.6

* TACs for 2017 and 2018 are fixed
2017 TAC is 23155t (East) and 2000t (West)
2018 TAC is 28200t (East) and 2350t (West)

* A bug was fixed where indices were recalibrated incorrectly among update years (causing artificial oscillation)

* The Perfect_Obs observation model now includes very small observation error (e.g. CVs ~ 1E-10 for most data types)

* A new performance metric (class PM) is available called AvC30 which is the average yield in the first 30 years

* PPlot and getperf have been updated to include AvC30


## Version 2.5

* Important error #1 was identified in which future indices were simulated to be identical if no hyperdepletion/hyperstability was detected. Many thanks to Mitsuyo Miyagawa for finding that one!

* Important error #2 was identified in which TACs by management area were being smeared over geographic areas due to a mistake in the allocation equation. Many thanks to Alex Hanke and Mitsuyo Miyagawa who both reported the issue. 

* The default plot(MSE) has been updated to show catches and SSB by area (East/West) rather than stock. You can set an argument 'byarea' = False to plot by stock. 

* Added a function Deterministic() that removes all process errors (recruitment deviations) from the projections. Coupled with the Perfect_Obs observation model this generates idealized projections for theoretical testing of MPs.

* The current (2018) TAC is by default 28200t in the East and 2350t in the West (this is an argument 'curTAC' in newMSE()). This is now provided to the simulate dataset 'dset' and used by the current catch reference MPs (CurC50, CurC100, CurC150).


## Version 2.4

* A series of memory optimization changes have been implemented that have reduced RAM resource requirements by around 3Gb for any MSE run. It still may be difficult to run the MSE on a system that does not  have at least 12 Gb of RAM however. 

* The current TACs for the East and West have been updated to 28200t (2018) and 2350t (2018-2020), respectively.

* Documentation has been added to CDD_i7 and XSA MPs, including a clarification that XSA is still not stable and in development. 
...
* All 36 operating models of the reference set are available in the package (OM_1, OM_2, etc)

* The four primary robustness operating models are also now available (OM_R1, OM_R2, etc)
...
The current operating model assumes a 7-area spatial structure:
...
The Atlantic bluefin tuna MSE follows an overarching design document coordinated by Doug Butterworth [Trial Specifications](https://drive.google.com/open?id=1vaGsXJfR4OFEwyx1QUX8KFEY8NUzgaYN) which details the reference and robustness scenarios for the MSE, the equations of the operating model and a set of preliminary performance metrics. 

A very brief introduction to the ABT-MSE R package is provided in [SCRS/2017/225](https://drive.google.com/open?id=1IBcZhxNJXwSKkmEoAFoGrj7BhzilNt81)

The MSE design (range of OMs) is described in [SCRS/2017/223](https://drive.google.com/open?id=1oJmXDD6tRKbEI4DwZnsLbFIAcq1EfiPt)

Examples of MPs coded and in action can be found in [SCRS/2017/224](https://drive.google.com/open?id=1gauaI74rowuzY3pAE-qFFlWQuY9wElPI)



## Data 

All of the data that are used in the operational modelling are available [here](https://drive.google.com/open?id=1mLVS_fqjNZMqP5sn3bdiFmWqrW9Sd1yX)

A meta-data summary of all data available to the ABT-MSE project is available online [here](https://drive.google.com/open?id=13pFaM3BTnzQ1BNQGoYn4O2n1IeD18V3VTbN9Hv7139U)

A description of the various types of data and their role in operational modelling can be found [here](https://drive.google.com/open?id=1TIkG5od71tlPMJuC7ejf5iTyYuTceEUv)
...
The ABT-MSE framework (data, development scripts, all code and the R Package) is freely available online from a dedicated GitHub code repository (www.github.com/iccat/abft-mse) and is also hosted on a [Google Drive folder](https://drive.google.com/open?id=0B0TXcs-MLRl3c0w4VF9QbkJYOU0). The GitHub repository allows users to request software features, report bugs, and track software updates. GitHub also supports continuous integration of code from multiple developers and feature ‘branching’ a form of version control where additions can be coded in parallel and merged with the master branch. The principal challenge of developing MSE for bluefin was  adopting a language and a programming paradigm that was sufficiently flexible to allow for increasingly complex additions. 
...
3. Download the ABT-MSE file structure from [here](https://drive.google.com/open?id=0B0TXcs-MLRl3c0w4VF9QbkJYOU0)
...
5. Install the ABTMSE R package by opening RStudio and entering the following code at the R console (making sure to change the file path to reflect where you put the ABT-MSE folder on your computer):
```{r install_package,eval=F}
install.packages("C:/ABT-MSE/ABTMSE_4.1.1.tar.gz") 
```
6. Check that the installation is successful by finding this help file (the ABTMSE vignette):
...
An MSE design object *Design* was also loaded with loadABT(). *Design* is a list of the factors and their levels used in defining the reference set of operating models, e.g. the MSE design outlined in the [Trial Specifications document](https://drive.google.com/open?id=1vaGsXJfR4OFEwyx1QUX8KFEY8NUzgaYN) .

Here is a figure showing how the reference OM numbering relates to the three factors (recruitment, abundance, spawning fraction / natural mortality rate M):

<img src="Ref_OMs.jpg" width="460">
...
This design grid explains the nomenclature of the various operating models (class OM) and their input definition files (class OMI) which are numbered from 1 to 24 corresponding with the rows of this design matrix:

```{r design_list_OMs}
avail('OM')
```

You'll notice that some objects in the ABTMSE package have the extension *\_example* and *\_d*. The example objects are stream-lined objects that are suitable for tutorials and demonstrations which involve less computational overhead (e.g. they require fewer simulations and require less system memory). The 'd' objects are deterministic operating models that simply have two simulations in them that are identical and fixed to the maximum likelihood values of the operating model fits. These are useful for MP testing since they provide a quick evaluation of MP performance at the most probable combination of operating model parameters. 


### Robustness Trials

Additionally a set of robustness OMs are specified that are single-factor deviations from 1AI (OM_1) and 2AI (OM_2):

<img src="Rob_OMs.jpg" width="460">
...
These are real abundance indices proposed for use by management procedures (the raw data are available [here](https://drive.google.com/open?id=1bgC-dKtETACGJxx4fQRwvhC2xBTnKsua)). These real indices are compared with operating model predictions to characterize their statistical properties so that realistic data can be generated in future projections. 

In the *Bad\_Obs* model there are currently `r max(Bad_Obs@MPind$No,na.rm=T)` indices. Note that many MPs that use indices to set TACs, will operate on one of these specific indices. For example you could have a delay difference model using index 3, (*DD\_i3*) and another using index 7 (*DD\_i7*).
...
To specify management procedures in this MSE framework you must select a MP for each spatially distinct area to which an MP is to be applied. For example, a VPA stock assessment in the West and a Surplus production stock assessment in the East. Traditionally the spatial division for Atlantic bluefin has been at 45 degrees W. In this quick start section we're going to ignore MP areas and the package will assume the default setup which is 2 data areas - 2 MPs, one per area, divided by 45deg W.  

A related issue is fleet allocation of regional TACs that are calculated by MPs. Again we're going to leave allocation for a later section of this document as it is a bit complicated. 
...
A very large amount of information has been stored in the MSE object *MSE_example*. There are many ways to analyze these simulated data. 
...
Each row of this plot represents a pair of MPs (a management system). Projections of catch and SSB are shown in terms of their median (solid black line), 5th and 95th percentiles (the shaded region) and ten individual simulations (the multicolored lines). 

Note that among MPs, each simulation has identical future biological conditions (recruitment, growth etc). In other words a red projected line (a simulation) for one MP corresponds with the red projected line for the other MPs. By keeping background simulation conditions the same among MP runs, the relative performance of MPs is revealed in much fewer simulations (one MP cannot outperform another due to sampling a more productive future by chance). 
...
Although it may be desirable to quantify management objectives in the initial phase of MSE (Punt et al. 2015). In the context of Atlantic bluefin tuna this is less appropriate since MSE is relatively new in this setting and requires iterative engagement from managers and stakeholders to shape the analysis and interpret outputs (ie. Nakatsuka 2016). 

While prioritization of management objectives has not yet been established in this early phase of the ABT MSE, a number of tentative performance metrics have been outlined in the [Trial Specifications document](https://drive.google.com/open?id=1vaGsXJfR4OFEwyx1QUX8KFEY8NUzgaYN)

<a id="step2"></a>

## Step 2: Quantification of management objectives

Following the [Trial Specifications document](https://drive.google.com/open?id=1vaGsXJfR4OFEwyx1QUX8KFEY8NUzgaYN) there are currently `r length(avail('PM'))-1` performance metrics that are used to evaluate management procedures. These are functions of class 'PM' (performance metric) and can be listed using the *avail()* function. 
...
A performance metric function has particular attributes. Functions of class PM accept an object of class MSE (argument *MSE*) and an integer value (argument *pp*) representing the number of a stock in the MSE object (stock 1 is the East, stock 2 is the West). The PM function returns a matrix that has a row for each MP in the model and a column for every simulation. 

For example, the probability of overfishing metric *POF* calculated for the `r MSE_example@nMPs` MPs and `r MSE_example@nsim` simulations for stock *1* (East) of the example MSE object *MSE_example*: 
...
The estimation 'engine' behind the ABTMSE R framework is M3 (short for modifiable multistock model), a multi-stock population dynamics model written in ADMB. Your ABT-MSE folder contains a subfolder "Manuals\_and\_design_documents" that contains further information about M3 including model equations and software design documents (they can be found online [here](https://drive.google.com/open?id=1FnOXvaM9V71G3rO_jiQVVg3ljjNkmQcM)). 
...
To make it easier for other users to modify the operating model data and assumptions, a new class of data object was designed. Named *OMI* (short for 'Operating Model Input'), this object class contains all the data in the correct format for input to the M3 model. This organisation of data allows for simple and rapid changes to OMI class objects and m3 model fitting, with the minimum of code. 
...
Lets make a change to the maturity schedule of the two stocks to demonstrate how a new operating model could be run. The OMI object we're going to look at is *OMI\_4*. 

The MSE design object (looking at Design\$Design\_Ref and Design\$all\_lnams) shows that operating model 4 is specified at the first level of the first factor (future recruitment = 1), the second level of the second factor (SSB level = B) and the first level of the third factor (maturity-mortality = I):
...
matplot(t(OMI_4@mat[,,30]),col=c('blue','red'),type='l',ylab="Maturity",xlab="Age",main="Previous maturity, OMI_4")
legend('bottomright',legend=c("East","West"),text.col=c('blue','red'),bty='n')
```

In this example we're going to copy the *OMI\_4* object, and change maturity to a logistic model of selectivity with an age at 50% maturity (inflection point) of 7 in the East and 10 in the West, and slope of 1.5 for both stocks:


```{r Step_3_OMI_7_new_mat,fig.width=4, fig.height=4}
OMI<-OMI_4 # copy the object
...
```{r Step_3_write_M3,eval=F}
M3dir<-"C:/Users/tcarruth/Dropbox/abft-mse/M3"   # Or whereever you put the /ABT-MSE/ directory on your computer
M3write(OMI,M3dir)
```
...
For each of your operating model folders (that now have an M3.exe and stats.cxx file in them) you can use the M3write() function above to write an M3 data file from unique OMI objects. Once each folder has an M3.exe, stats.cxx and M3.dat file you can run the M3 model.  
...
```{r Step_3_run_M3_parallel,eval=F}
 ...
 M3dirs
 sfInit(parallel=TRUE, cpus=detectCores()) 
 #sapply(1:length(M3dirs),runM3p,OMdir=M3dirs)  # Lets not actually do this because it will take ages!
...
This has produced an operating model with the default settings, namely 48 simulations, future projection of 50 years, recruitment based on Beverton-Holt dynamics and default naming of stocks ("East" is stock 1, "West" is stock 2). 

The operating model object has read the M3 outputs and assigned parameter values based on the posterior samples generated by the Metropolis-Hastings algorithm of ADMB 
...
An important axis of uncertainty in this MSE is future recruitment. This is described in the [Trial Specifications document](https://drive.google.com/open?id=1vaGsXJfR4OFEwyx1QUX8KFEY8NUzgaYN) and the MSE *Design* object:
...
In the West, we have a hockey stick stock-recruitment model fitted after 1975 and a Beverton Holt stock-recruitment model fitted prior to 1975. For projections, these start by assuming the hockey stick recruitment dynamics and then change to the Beverton Holt model after 10 projected years. 

Things are more complex in the East. Projections start with Beverton Holt recruitment fitted to model predictions from 1988 onwards but with a fixed steepness of 0.98. After ten projected years this changes to the Beverton Holt stock recruitment model fitted to 1950-1987 recruitment/SSB estimates also with a fixed steepness of 0.98. 
...
We're going to go through each list element one-by-one. 

The first list item is called *$proyears*. This defines when recruitment changes in the future. *proyears* is an array with n time period rows (in this case there are two projected time periods: projection years 1:11 and 12 onwards) and 2 columns showing when these projection time periods start and end. 
...
The second list item is call *$SRno* and this defines the number of the SR curve estimated in the M3 model. In this case 1 is East til 1987, 2 is East after 1988, 3 is west til 1974, 4 is west after 1975.  

Column 1 specifies the two future recruitment scenarios for the East, Column 2 specifies the two stock-recruitment scenarios for the west.

The *$type* list item simply specifies what kind of stock-recruitment relationship is used in each case. In this case it can be one of two types

 * BH: Beverton-Holt with steepness fixed and and unfished recruitment estimated
 * HS: Hockey-Stick with the inflection point fixed to mid 1990s SSB and unfished recruitment estimated
...
The fourth list item *fixpar* simply specifies the necessary fixed parameters, in this case steepness of 0.98 for both time periods for the Beverton Holt stock recruitment relationships of the Eastern Stock. Note that for the Hockey stick model this is not applicable. 
...
Additionally every time an MSE is run, the most current simulated dataset is passed to the global environment (called *dset*). It follows that if the MSE function crashes due to an error in an MP, it is easier to test the MP again using the offending simulated data and debug the issue (note that the dset you see if the MSE crashes is a list object so dset[[1]] is the equivalent of dset\_example\_East, for example.  
...
Lets look at a relativey simple MP, Islope1 which can have variants depending on which index you want to use (below is the example that uses index number 3):
...
Note that for simple MPs like this, parallel processing isn't necessary and might even be a bit slower due to the overhead of sending data to the cluster. However for more complex MPs that involve estimation phases, parallel processing can speed up the analysis by a large margin (typically running in 1/(0.8*nthreads) the time). 
...
Many MPs use numerical optimization to fit models to data. A good example are stock assessments such as surplus production models that are still used to provide management advice for Atlantic tunas and billfish, for example BSP (McAllister et al. ) and ASPIC. 
...
In this case we're going to create an MP for the Western Stock using the GOM_LAR_SUV index. Remember these indices are stored in the @MPind slot of the observation model:
...
SP_i7<-function(x,dset,startD=0.5,checkfit=F){                  # a very simple surplus production model, r, K and q leading
  
  nyears<-dim(dset$Iobs)[3]               # get the number of years of data (the 3rd dimension of Iobs)
  Ind<-dset$Iobs[x,7,1:nyears]            # get the index
...
```{r Step_4_SP_test1a}
 SP_i7(x=1,dset=dset_example_West)
 SP_i7(x=1,dset=dset_example_West,startD=0.5)
...
```{r Step_4_SP_test1b,fig.width=4,fig.height=4}
 SP_i7(x=1,dset=dset_example_West,checkfit=T)
```
...
```{r Step_4_SP_test2,error = TRUE}
 nsim<-nrow(dset_example_West$Cobs)
 sapply(1:nsim,SP_i7,dset_example_West)
```
...
```{r Step_4_SP_test3a,error = TRUE}
 sfInit(parallel=T,cpus=detectCores())
 sfSapply(1:nsim,SP_i7,dset_example_West)
```
...
```{r Step_4_SP_test3c}
 system.time(sapply(1:nsim,SP_i7,dset_example_West))
 system.time(sfSapply(1:nsim,SP_i7,dset_example_West))
...
You may be reading this document because you wish to design and test a management procedure that sets annual TACs for the Eastern or Western management areas using just indices of abundance. This is a critical step in the MSE process for ABFT - do not hesitate to contact Tom Carruthers (t.carruthers@oceans.ubc.ca) for help with MP development.  

In this example we are going to specify a very simple target index MP, an MP that will seek a target level by modifying TACs. 

There are a number of possible ways to do this but we will use a very basic incremental TAC stepping method.

We need to know a few things including 1) the target level of the relevant index, 2) the maximum rate of TAC change we are willing to tolerate.

We can see the names of the various indices available for MP development by looking at the table object *Indices* that was loaded when your ran the line loadABT(). 

```{r MPdev1}
Indices

```
You will notice that each index is collected in an area associated with a current stock assessment where Stock = 1 is the Eastern Stock and Stock = 2 is the Western stock. 

Lets say we wished to use the Mediterranean Larval Survey (Index No. 3) to set TACs in the East and the Gulf of Mexico Larval Survey (Index No. 7) to set TACs in the West. 

[Current multispecies operating model fits](https://drive.google.com/open?id=1oJmXDD6tRKbEI4DwZnsLbFIAcq1EfiPt)  indicate that current stock levels (over the last three years) are around 1.5 times MSY levels in the East ([Figure 4 for base reference operating model 1AI of this document](https://drive.google.com/open?id=1oJmXDD6tRKbEI4DwZnsLbFIAcq1EfiPt)) and around 1.3 times MSY levels in the West. 
...
```{r MPdev2}
MPind<-Bad_Obs@MPind
IndE<-MPind[MPind$No==3 & MPind$Year%in%(50:52),] # get the Med Larval Survey
IndW<-MPind[MPind$No==7 & MPind$Year%in%(50:52),] # get the GOM Larval Survey
...
TargW=mean(IndW$Index)/1.3
paste("TargE =",TargE)
paste("TargW =",TargW)

```

In this simple example, the two index based MPs will alter the current TAC according to the ratio of current index levels (the observed index over the last *yrs4mean* number of years (default is 4 years) to target levels. We will set the maximum rate of TAC change (Delta) to 15% for both MPs.
...
The Western MP is only different in the first line of function where we have to set an appropriate target level and index number:
...
We can now test out our new MPs against simple mean catches and UMSY fishing:
...
MCD_i5<-function(x,dset,startD=0.5){
  
  nyears<-dim(dset$Iobs)[3]                  # Most recent year
  mean(dset$Cobs[x,],na.rm=T)*               # Average historical catches
    mean(dset$Iobs[x,5,(nyears-2):nyears]/max(dset$Iobs[x,5,],na.rm=T))*   # Mean index over last three years
...
* It is generally a good idea to take measures to ensure your MPs are as robust as possible. For example when taking average catches it could be a good idea to ignore 'NA' values. For example, setting the argument *na.rm* in our mean catch MP:
...
New to version 3.2 is a standardized MSE report which you can generate for any MSE object
...
Lets remind ourselves of the 7 area stock structure of the current operating models and MSE analyses:
...
* Area 1 (Eastern): SATL, NATL, EATL and MED (areas 4-7)
* Area 2 (Western): GOM, WATL, GSL (areas 1-3)

To represent this in an MSE analysis we create a vector n areas long with the right area designation in each position (areas 4-7 are area 1, areas 1-3 are area 2).


```{r MPareas}
MPareas<-c(rep(2,3),rep(1,OM_example@nareas-3))
MPareas
```
...
Specifying allocation among fleets requires a matrix that has a row for each of the seven areas and a column for each fleet. These fractions add up to 1 for the areas that make up the East and West TACs. Ie rows 1-3 (GOM, WATL and GSL) sum to 1 over all fleets. 
...
This Allocation matrix is an argument to the MSE run. E.g:

```{r Allocation2, eval=F}
myAllocation<-Allocation
myAllocation[1,]==0 # no catch in the GOM
myAllocation[1:3,]<-myAllocation[1:3,]/sum(myAllocation[1:3,]) # renormalize to sum to 1 in western areas
myMSE<-new('MSE',OM_1d,MPs=list(c("UMSY","UMSY")),Allocation=myAllocation)
```
...
```{r plot_TEST, fig.width=6,fig.height=7,eval=F}
...
out<-M3read("C:/Users/tcarruth/Dropbox/abft-mse/M3") # get the raw M3 model fit 
load("C:/Users/tcarruth/Dropbox/abft-mse/M3/OMI")    # load the corresponding OMI object in the same folder
